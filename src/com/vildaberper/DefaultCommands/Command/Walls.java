...
public class Walls{
	public static boolean walls(CommandSender sender, Command command, String label, String[] args){
		if(sender instanceof Player){
			if(!Perm.hasPermission((Player) sender, "dc.walls")){
				return false;
			}
			if(args.length == 1){
				if(Misc.getSelection(((Player) sender).getName()) != null){
					List<Block> blocks = Edit.getWalls(Misc.getSelection(((Player) sender).getName()).getBlock1(), Misc.getSelection(((Player) sender).getName()).getBlock2());
